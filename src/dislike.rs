...
pub fn is_disliked(hct: &Hct) -> bool {
    let (hue_passes, chroma_passes, tone_passes) = if cfg!(feature = "std") {
        (
        ...
        (
            (90.0..=111.0).contains(&libm::round(hct.get_hue())),
            libm::round(hct.get_chroma()) > 16.0,
            libm::round(hct.get_tone()) < 65.0,
        )
    };
